...
from mmengine.fileio.io import load as load_file
...
def load_checkpoint(model, filename, map_location="cpu", strict=False, logger=None):
    ...
    for table_key in relative_position_bias_table_keys:
        ...
        new_table_key = ".".join(
            table_key.split(".")[:-1] + ["module"] + [table_key.split(".")[-1]]
        )
    ...
    for key, value in state_dict.items():
        new_key = ".".join(key.split(".")[:-1] + ["module"] + [key.split(".")[-1]])
        new_key_2 = ".".join(key.split(".")[:-2] + ["module"] + key.split(".")[-2:])
        new_state_dict[new_key] = value
        new_state_dict[new_key_2] = value
